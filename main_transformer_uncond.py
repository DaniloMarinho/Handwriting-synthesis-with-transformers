...
# n_heads = 4
n_heads = 8
hidden_size = 400  # must be even to use pos encoding
n_layers = 2
...
# src_mask = TransformerModel.generate_square_subsequent_mask(max_seq+1).unsqueeze(0).repeat(batch_size, 1, 1).to(device)
...
for big_epoch in range(epochs):
    ...
    for i in range(0, num_mini_batch, batch_size):
      ...
      # NOT OPTIMAL - TODO: make sqeuence-wise form below work
      loss = 0
