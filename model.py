...
def mdn_loss(mdn_params, data, mask=[]):
    ...
    # print(mdn_params.shape, data.shape)
...
def sample_prime(lr_model, text, start_text, start_stroke, char_to_vec, hidden_size, time_step=2000, scale = 50,\
                rnn_type = 2, bias1 = 1, bias2 = 1, num_attn_gaussian = 10, bi_dir=True, random_state= 56):
    np.random.seed(random_state)
    ...
    prev_x = torch.tensor(start_stroke, dtype=torch.float, device=device)[0]

    strokes = np.zeros((time_step, 3), dtype=np.float32)
    old_k = torch.zeros((1,num_attn_gaussian), dtype=torch.float, device=device)
    text_len = torch.tensor([[len(text_concat)]], dtype=torch.float, device=device)
    ...
    for i in range(time_step):
        ...
        if i < len(start_stroke):
            prev_x = torch.tensor(start_stroke, dtype=torch.float, device=device)[i]
            zero_timestep = i
