...
TODO:
* compare loss to a simple baseline for generating mixture parameters (e.g. MLP)
* calculate mdn loss sequence-wise instead of looping
...
* try enforcing representations on polar coordinates
...
class TransformerModel(nn.Module):
    def __init__(self, input_size, n_heads, hidden_size, n_layers, dropout=0.2):
...
class MixtureWeights(nn.Module):
    def __init__(self, n_heads, hidden_size, n_layers, num_gaussian, dropout=0.2):
        super(MixtureWeights, self).__init__()
        self.model_type = "Transformer"
        self.pos_emb = PositionalEncoding(hidden_size,dropout)
        decoder_layers = nn.TransformerDecoderLayer(hidden_size,n_heads,hidden_size,dropout,batch_first=True)
    ...
    def forward(self, tgt, memory, src_mask):
        # output = self.pos_emb(src) * np.sqrt(self.hidden_size) 
        # output = self.transformer_decoder(output, src, tgt_mask=src_mask)

        tgt = self.pos_emb(tgt) * np.sqrt(self.hidden_size) 
        output = self.transformer_decoder(tgt, memory, tgt_mask=src_mask, memory_mask=src_mask)
...
class ModelUncond(nn.Module):
    def __init__(self, input_size, n_heads, hidden_size, n_layers=2, num_gaussian=2, dropout=0.5):
        ...
        self.decoder = MixtureWeights(n_heads, hidden_size, n_layers, num_gaussian)
        
    def forward(self, src, src_mask):
        # base model
        x = self.base(src, src_mask)
        # MDN parameters
        mdn_params = self.decoder(src, x, src_mask)
        return mdn_params
    
class ModelCond(nn.Module):
    pass

# NOT OPTIMAL - TODO: make sqeuence-wise form work
def mdn_loss_transformer(stroke, mdn_params, data, mask=[]):

    def get_2d_normal(x1,x2,mu1,mu2,s1,s2,rho):
      
    #   print(x1.shape,x2.shape,mu1.shape,mu2.shape,s1.shape,s2.shape,rho.shape)
    ...
    # print(mdn_params[0].shape, data.shape)
...
#       ##### implementing Eqn. 24 and 25 of the paper ###########
#       norm1 = torch.sub(x1.view(x1.shape[0],-1,1),mu1)
#       norm2 = torch.sub(x2.view(x2.shape[0],-1,1),mu2)
#       s1s2 = torch.mul(s1,s2)
#       z = torch.div(norm1**2,s1**2) + torch.div(norm2**2,s2**2) - 2*torch.div(torch.mul(rho, torch.mul(norm1,norm2)),s1s2)
#       deno = 2*np.pi*s1s2*torch.sqrt(1-rho**2)
#       numer = torch.exp(torch.div(-z,2*(1-rho**2)))
...
#     res1 = -torch.log(torch.max(res1,epsilon))
#     # res2 = torch.mul(eos, e_t.t()) + torch.mul(1-eos,1-e_t.t())
#     res2 = torch.einsum('bik,bjk->bij', eos.unsqueeze(0), e_t) + torch.einsum('bik,bjk->bij', 1-eos.unsqueeze(0), 1-e_t)
#     res2 = -torch.log(res2)
...
def sample_uncond(model, start=[0,0,0], time_step=600, scale = 20, random_state= 98):
    ...
    for i in range(time_step):
        ...
        # print(prev_x.shape, src_mask.shape, prev_x.type)
        # mdn_params = model(prev_x.unsqueeze(0),src_mask=src_mask)
